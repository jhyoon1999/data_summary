...
with tab1 :
    ...
    st.image("src/ACT/project_purpose.png", use_column_width="auto") ##추후 수정##

    st.markdown('<hr style="border: 1px solid #ccc; margin: 20px 0;">', unsafe_allow_html=True)
    
    st.subheader("Ⅲ. 프로젝트 프로세스")
    st.image("src/ACT/project_process.png", use_column_width="auto")
...
with tab2 :
    ...
    with fcol2 :
            ...
            st.markdown("""
                        - 데이터 구조 파악을 위한 기초통계치 산출
                        - 적응적 검사 개발의 일반적 접근인 문항반응이론의 가정 충족 여부 확인 → 불충족 확인
                        - 적응적 검사의 형태를 갖는 머신러닝 모델 의사결정나무(Decision Tree) 기반 검사 시스템 개발 결정
                        """)
            st.image("src/ACT/assumption_result.png", use_column_width="auto")
    ...
    with scol2 :
            ...
            st.markdown("""
                        - Yan(2004)이 제시한 Merged Decision Tree 알고리즘이 의사결정나무의 과적합 발생을 억제할 수 있음을 발견
                        - 구현 언어 : R
                        """)
            st.image("src/ACT/merged_tree.png", use_column_width="auto")
    ...
    with tcol2 :
            ...
            st.markdown("""
                        - 각 인지영역 별로 모델 성능 비교 후, 최종 모델 선정
                        """)
            st.image("src/ACT/development_ACT.png", use_column_width="auto")
    ...
    with focol2 :
            st.text('')
            st.markdown(f'<div style="position:relative; text-align:left;">\
                        <p style="font-weight:bold;">{"목적 :제품 출시를 위한 개발 보고서, Working Technical Document 작성"}</p>\
                    </div>', unsafe_allow_html=True)
            st.image("src/ACT/document_img.png", use_column_width="auto")
